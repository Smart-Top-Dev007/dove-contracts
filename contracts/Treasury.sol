...
contract DoveTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SDOVE }
    ...
    uint public sDOVEQueue; // Delays change to sDOVE address
    ...
    constructor (
       ...
       isLiquidityToken[ _DOVEDAI ] = true;
       liquidityTokens.push( _DOVEDAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for DOVE
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint DOVE needed and store amount of rewards for distribution
    ...
    /**
        @notice allow approved address to burn DOVE for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IDOVEERC20( DOVE ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sDOVE ).balanceOf( msg.sender ); // Can only borrow against sDOVE held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with DOVE
        @param _amount uint
     */
    function repayDebtWithDOVE( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IDOVEERC20( DOVE ).burnFrom( msg.sender, _amount );
    ...
    /**
        @notice returns DOVE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match DOVE decimals
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SDOVE ) { // 9
            sDOVEQueue = block.number.add( blocksNeededForQueue );
        } else return false;
