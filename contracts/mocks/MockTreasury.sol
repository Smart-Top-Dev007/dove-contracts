...
contract MockDoveTreasury is Ownable {
  using SafeMath for uint256;
  ...
  uint256 public sDOVEQueue; // Delays change to sDOVE address
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sDOVE).balanceOf(msg.sender); // Can only borrow against sDOVE held
  ...
  /**
        ...
        @param _amount uint
     */
  function repayDebtWithDOVE(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(DOVE).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SDOVE) {
      // 9
