...
contract DoveCVXBondDepository is Ownable {
    ...
    address public immutable treasury; // mints DOVE when receives principal
    ...
    struct Bond {
        uint payout; // DOVE remaining to be paid
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( DOVE ).safeTransfer( _recipient, _amount ); // send payout
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principal or DOVE) to the DAO
     *  @return bool
     */
